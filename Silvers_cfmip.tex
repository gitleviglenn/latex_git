...
A systematic picture will be presented of the simulated clouds across a hierarchy of CMIP6 model configurations  that have been run with 
the AM4 and CM4 climate models from GFDL.  This hierarchy is composed of both CMIP6 and CFMIP (Cloud Feedback Model Intercomparison Project) experiments that include an ocean covered planet, atmosphere/land only Earth-like experiments, and fully coupled model experiments both of the historical era and future warming scenarios (\textit{aqua-control, amip, historical, amip-p4K, amip-future4K, abrupt-4xCO2}, and \textit{1pctCO2}).   
We demonstrate the large range of climate sensitivities that can result from a single parent atmospheric climate model.   
Despite a relatively low top-of-atmosphere sensitivity to uniform warming of SSTs 
(Cess-type warming experiments), the corresponding coupled climate model, CM4, has high transient (2 K) and 
equilibrium climate sensitivities (~ 5K).  Cloud feedbacks play a large role in setting these sensitivities.  
...
experiments relate to the fully coupled experiments and to the clouds observed in Earth's atmosphere.  

Observations of cloud fraction from multiple satellite products (CALIPSO, MISR, MODIS, and ISCCP)  are compared to the clouds simulated by the AM4 and CM4 climate models.  
The AM4 and CM4 climate models succeeded at significantly reducing the TOA radiative flux biases as compared 
to CERES-EBAF observations.  Partly as a result of this realistic representation of the TOA energetics the coupled model
improved the representation of precipitation, reduced the Intertropical Convergence Zone bias, and produces an excellent El Nino-Southern Oscillation.  However, we find large biases in cloud fraction relative to observations across wide regions of the globe and throughout the depth of the atmosphere.    

I will discuss some of the challenges that are inherent to improving cloud simulations in GCMs.
This work points to the need for drawing dependable cloud constraints from the observations we have.  The important challenge will then be to implement the cloud constraints while continuing to maintain a realistic representation of the radiative fluxes at the TOA of global models.  
...
%During the development of AM4.0 version 2.8 of CERES-EBAF was used extensively \cite{Zhao_etal_2018a}.  
