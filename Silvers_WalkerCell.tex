...
There are circulation cells below $500 \rm{hPa}$  and above $500 \rm{hPa}$.  Deep-convective activity dominates the 
...
We perform a series of sensitivity experiments to highlight the different ways in which these experiments can equilibrate.  To 
show the sensitive nature of the distribution of the precipitation maximum we test the impact of convective parameterization, 
...
\subsection{The Influence of Domain Size on the LWCRE and the LS Parameterized Precipitation}
...
long horizontal dimension fixed at $4000\, \rm{km}$.  However, previous studies (e.g. Bretherton et al. 2006) have 
...
Can we hope to simulate all of this in an idealized modeling context?    
Our initial motivation for using the GFDL AM4 model on this doubly-periodic domain was to simulate this region 
%that is rich in phenomena 
in the context of both a GCM and a CRM in the hopes of learning something from the high resolution 
cases that will help us to improve the development of the GCM.    
...
overturning circulation at different resolutions.  Shown are 180 days precipitation from the P25, E25, E2, and E1 simulations.  As the 
...
times, compared to these first 180 days.  The simulations with explicit convection at resolutions typical of cloud-resolving 
...
lowest atmospheric level.  It is also worth noting that the E1 experiment has a stronger domain mean u wind shear compared 
...
strengthens as the resolution increases.  When the clouds and radiation are allowed 
to directly interact with each the E2 and E1 simulations appear much more similar (Figure \ref{fig:rh_psi_P25E2E1}).   
This dependence of circulation strength and moisture on resolution is especially 
apparent when the LWCRE is off (Figure \ref{fig:rh_psi_P25E2E1_lwoff}).     
...
The low-level clouds of the P25 and P100 experiments are dramatically reduced when the LWCRE is off.  For 
