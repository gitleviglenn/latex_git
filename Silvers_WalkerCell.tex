...
%% We analyze an idealization of this circulation with experiments that use a range of grid-spacing from 1km to 100km.
%% These experiments allow us to examine the multi-scale interactions between circulations, cloud systems, and radiation.
%% Our experiments span the typical range of resolutions between General Circulation Models (GCMs) and 
%% Cloud-system Resolving Models (CRMs).
...
%precipitation for our GCMs, but decreased precipitation for our CRMs.  Decreasing the resolution to 1km and 2km
...
%a link between the simpler models 
...
%The LWCRE is an important element for all of the experiments and will be discussed throughout the paper.  
...
%radiation will be discussed in later sections (also be seen in Figures 4, 5, 9 and 10), and is particularly apparent for 
...
%The added constraint (relative to RCE) of the fixed warm-patch of SST could lead to clarification of the processes that drive important atmospheric interactions.
...
%The SF indicates not only whether an experiment is aggregated, but also the spatial extent of the overturning circulation.  
...
%\textit{Previous studies have shown that Cloud Radiative Effects (CREs) act to strengthen and contract an 
%overturning circulation \cite<e.g.>[]{Fermepin_Bony_2014, Harrop_Hartmann_2016b, Popp_Silvers_2017,Albern_etal_2018}.  
%This is due to an increased low-level flux of moist static energy into the convective regions. }
...
%two of the most fundamental choices.   
%One of the goals of this study was to explore the results of changing grid-spacing over a wide range of values.  
...
%This is consistent with previous work showing that CREs 
%strengthen an overturning circulation and narrow the region of deep convection (see \citeA{Harrop_Hartmann_2016b}; 
%\citeA{Popp_Silvers_2017}; \citeA{Albern_etal_2018}; \citeA{Dixit_etal_2018}  etc.).  
...
%As previously noted, these GCM-like simulations equilibrate after approximately 50 days.  
...
%This can be seen in the domain mean total condensate in Figure \ref{fig:TotCond_P25P100}.  The  low- level maximum of 
%condensate is shifted from near 900 hPa in the small domain (thin lines) to between 700-800 hPa in 
%the large domain (thick lines).  
...
%Overall the large domains show an upward shift of condensate and have 
%a much stronger upper level response to the LWCRE being turned off.
%For the LWCRE-off cases, in agreement with \citeA{Bretherton_etal_2006} we find a more 
...
%and P25L but for P100 and P100L the ascent region is much more focused on the large domain (Figure \ref{fig:conv_vs_ls}).    
...
%is that although the deep and shallow convection are parameterized
...
obvious difference being the humidity in the center of the domains where the RH differs by as much as 40\%. % among E1, E2, and E25.  
%The mid-troposphere 
...
%It also provides clues about the strength of convection and vertical mass transport.     
...
%with the former having much higher values of low-level liquid condensate and the later having much higher values of upper level ice condensate.  
...
% Is the paragraph below out of place?  As written, it references Figures 9, 10, and 13.  Should it be near the end of this section
% as a sort of summary of the effects of LWCRE?  I also don't currently like the last sentence.... It has potential but as written it 
% says nothing.   
...
This is highlighted by comparing experiments with and without the LWCRE. %  highlights key differences between our GCM and CRM experiments.  
...
%While the mid-tropospheric profiles of diabatic heating and cooling are similar between the LWCRE on/off simulations, there is a very strong 
%response below about 850 hPa for the 25km and 100km models.  
%At that grid-spacing, with the LWCRE on, significant low-level clouds are formed, leading to a radiative cooling on the order of -10K/day.   
%However, when the LWCRE is off the radiative cooling decreases to - 3-5 K/day and there is little appreciable 
%difference in the low-level diabatic profiles among the P100, P25, E25, E2, and E1 experiments.   
%It is also clearly seen that the LWCRE decreases the upper-level ice-condensate 
%for E25, E2, and E1 experiments but dramatically increases the low-level liquid condensate for E25 and P25 (Figure \ref{fig:TotCond}).  
...
%When this source of atmospheric energy loss is removed in the LWCRE-off experiments the upwelling LW flux of radiation plays a larger 
...
\section{Summary and Discussion} % and Summary of the Impact of the LWCRE and Grid-spacing on the Atmospheric State}
...
\begin{itemize}
  ...
  %\item The domain mean midtropospheric relative humidity increases by $5-15 \% $.  --> Not shown in current draft
 ...
 % \item Over the region with warm SST the surface enthalpy flux among models is in much better agreement, 
  %especially between the E1 and E2 experiments (Figure \ref{fig:enthalpy}).
...
\begin{itemize}
  %\item The domain mean precipitation response to LWCRE is opposite to that of the $100 \textrm{km}$ and $25 \textrm{km}$ experiments (Table 2).
  ...
  Between 300-800 hPa in the upwelling regions the E1 and E2 %$1 \textrm{km}$ and $2 \textrm{km}$ 
...
%This is not necessarily surprising and is likely the source of much of the differences among the experiments.  
...
%However, our study of the interaction between clouds, radiation, and the circulation shows how these interactions 
...
towards a global model \cite<e.g.,>[]{Schneider_etal_2017,Schneider_etal_2019}.
...
\citeA{Muller_Hohenegger_2020} would provide a context within which the 
