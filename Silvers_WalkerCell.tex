...
\begin{itemize}
  ...
  a forced compositing of the data (\cite{Bretherton_etal_2005} Bretherton et al. 2005, Muller and Held, 2012).}
...
($\overline{\rm{OLR}}$), and subsidence fraction (SF)  or fraction of domain that is subsiding at the 532 hPa level.
...
\begin{figure}
  \centering
      ...
      with a grid-spacing of 25km.  The lower panels show the total (liquid + ice) condensate (g/kg).  
...
(e.g. Popp and Silvers, 2017, HH, and others (Muller and Held?)).  This was shown to be due to an increased low-level flux
...
includes an additional constraint than RCE.  Given identical SSTs, the variations in SF give a measure of variability
driven entirely by the atmosphere.  
...
This can be understood as a consequence of the differences in upper level cloud fraction and the surface energy budget 
...
(e.g. Bretherton et al., 2006; Muller and Held, 2012; Jeevanjee and Romps, 2013; sens of what? Patrizio and Randall? Silvers?) have 
documented sensitivities to domain size.   The analysis of the previous, and of the next sections focus on results from 
...
spacing of $25\, \rm{km}$ on a domain with a horizontal width of $4000\, \rm{km}$ (far right) and $16000\, \rm{km}$ (middle right), and 
...
larger.   In contrast, here larger domains have larger precipitation rates (Table 2).  For the LWCRE off cases, we also find a more 
...
While the relative contribution is not well constrained, it is thought to influence the atmospheric state (Held et al., what part of the state?). 
...
but does act to spatially concentrate it.  With the single exception 
...
seen in Figure \ref{fig:domdep_lwoff} corresponds to decrease in the large-scale precipitation of about 65\% in P25L case
