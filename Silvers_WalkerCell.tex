...
%  noted by Sir Gilbert Walker, and described by \citeA{Bjerknes1969} connects the Indonesian region that is dominated 
...
pacific with the subtropics and has been extensively studied (review paper?).  The circulation first 
  ...
  (Geisler, 1981), or modeling (\citeA{Grabowski2000}, Bretherton et al., 2006; Wofsy and Kuang, 2012, 
  Kuang, 2012)).  The modeling studies have primarily used models referred to as Cloud-system 
  Resolving Models (CRM), 
  which has meant a grid-spacing of less than 5km and no convective parameterization.    One of the primary 
  motivations of this paper is to compare cloud resolving model simulations 
  of the Walker Circulation with simulations from a GCM within a flexible dynamical framework that draws on a 
  single code base for both models.  
...
circulation are the use of fully interactive radiation, and the direct comparisons with a state-of-the-art general circulation
...
off so that all of the convection in that experiment is explicit, as it is in E2 and E1.  Domains with dimensions of 16000 km where judged too 
costly for the 1km and 2km experiments.  Basic experimental details and the naming convection for the experiments are shown in Table 1.
...
implemented as part of the CFMIP series of experiments (Stevens et al. 2012, Webb et al., 2017).
...
SSTs and a humid boundary layer across the full length of the domain.    Large concentrations of condensate are 
present in the mid-troposphere over the warmer SSTs, while the regions with subsiding circulations are 
dry ($< 10\% RH$) above about $900 \rm{hPa}$.  The left panels of Figure \ref{fig:rh_psi_P25vsE25}  show that our 
control GCM experiment has a stronger, more concentrated circulation with more condensate in the convective
region and a dryer subsidence region, relative to the cases with the LWCRE off (middle panels) and with the
convective parameterization off (right panels).
...
This state has the same fundamental characteristics as the Walker circulation, the Hadley Cell, and 
...
cooler SST (Figure \ref{fig:rh_psi_P25vsE25}) than either E25 or P25 LWCRE off.  

Averaged over the full domain, the P25 case with parameterized convection results in a dryer atmosphere with less 
...
As the domain size increases, so does the domain mean precipitable water.  Active LWCREs lead
to larger values of barPW in all cases (Table 2).  (Does this tell us anything about precipitation efficiency?)  
It is also interesting to note that barPW varies by as much as 30\%.  

Superposing the circulation and relative humidity (as in Figure \ref{fig:rh_psi_P25vsE25}) illustrates the subsidence driven 
...
region over the warm pool, shallow convection is common over a wider range of SSTs, and for the most part 
stratocumulus clouds are absent (based on what?).  When the coupling between the circulation and clouds is 
broken, by turning off the longwave CRE, the atmospheric state is much more symmetric about the maximum 
SST.  We also find large differences among the experiments in the domain mean precipitation, condensate/clouds, 
and circulation (see Table 2).  In general the experiments with parameterized convection are much more erratic.   
...
shifted to slightly cooler temperatures.  This asymmetry is apparent in the vertical velocity, mass circulation, relative humidity, 
specific humidity, and radiative heating.  
In the Hovm{\"o}ller diagrams (Figures \ref{fig:domdep} and \ref{fig:domdep_lwoff}), the precipitation appears to be averse to 
...
parameterization and the LWCRE.   The degree to which this asymmetry influences the comparison with other experiments is 
unclear.    

Complex patterns of precipitation over a fixed sinusoidal or Gaussian SST distribution have been noted many times in 
previous literature \cite{Bretherton2006, Wofsy2012} (Ming Zhao, see Jeevanjee et al. 2018)
...
entirely explicit (E25, E2, and E1, discussed further in section 4), but strikingly different than for P25.      
...
(e.g. Popp and Silvers, 2017, Harrop and Hartmann, (Muller and Held?), Albern et al., 2019).  This is 
...
subsidence fraction (SF).  As convection becomes more organized, or aggregated, the SF will increase.  We expect 
...
dependence of aggregated atmospheric states on temperature (\citeA{Khairoutdinov2010}; \citeA{Wing2014}).  
While there does appear to be a dependence 
on temperature, Table 2 illustrates how much the aggregated 
state can vary among experiments with identical SST.  Having a prescribed SST warm 
patch ensures that the simulations will be `aggregated' to some degree.  Rather than indicating whether an 
experiment is aggregated, the SF here measures the spatial extent of the overturning circulation.  The SF provides a 
simple way of quantifying the degree of organization that is present.  Given identical SSTs, the  range of 
different SFs provide a measure of variability that is driven entirely by the interactions between 
...
Precipitation from each of these two components isshown in Figure \ref{fig:conv_vs_ls} as a function of 
...
We also demonstrate that it is through the longwave cloud radiative effect that this connection is enabled.
...
We now show results that resulted from our motivation to simulate a tropical Pacific-like region 
...
the LWCRE is off.  This suggests that the interactions between clouds and radiation help the 
...
distribution of condensate is closely related to the distribution of clouds.  This in turn influences the distribution and
flow of energy throughout the atmosphere and the longwave radiation that is emitted to space.  It can also indicate the 
...
GCM-like experiments.  The interactive LWCRE increases the amount of upper level ice-condensate 
...
surface enthalpy flux reveals large differences in the symmetry of the near surface energetics.   
...
flux for E1and E2 become very similar over the warmest SSTs.   Thus even for the case of prescribed SSTs and no
land surface the interactions between clouds and the longwave radiation have a massive influence on the 
...
P100 experiments.   \textit{The domain mean precipitation rates decrease despite an increased 
...
a source of effective atmospheric warming.   The large decrease of low-level clouds also leads to an increase of downward shortwave 
...
response below about 850 hPa for the 25km and 100km models.  At that grid-spacing, with the LWCRE on, significant low-level 
clouds are formed, leading to a radiative cooling on the order of -10K/day.   
However, when the LWCRE is off, there is little appreciable difference in the low-level diabatic profiles among the 
...
grid spacing and a threshold based `binary' cloud scheme), the prospect of so easily converting a GCM into something 
...
effect of clouds, with and without the convective parameterization, across multiple domain sizes and resolutions.  
...
20\% despite having the same prescribed SST and incoming radiation.  All simulations have the same radiation,
