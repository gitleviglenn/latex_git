...
We analyze this circulation with high-resolution 
and coarse resolution simulations and infer how deep convective heating in the west Pacific can influence the atmospheric cooling 
...
pacific with subtropics and has been extensively studies (review paper?).  The circulation first 
  noted by Sir Gilbert Walker, and described by \citeA{Bjerknes1969} connects the Indonesian region that is dominated by deep 
...
An additional change that results from increasing the domain width is an upward shift of the cloud fields.  This is 
...
the large-scale precipitation.  The LW CRE has a much smaller effect on the magnitude of the convective 
precipitation 
...
of circulation and humidity.   Figures \ref{fig:rh_psi_P25E2E1} and \ref{fig:rh_psi_P25E2E1_lwoff} show 
the humidity, condensate, and circulation with and without the LWCRE.  Both figures illustrate that the E1
and E2 experiments have a stronger deep overturning circulation above the warm patch and they have 
significantly more condensate aloft in this region.      
It is also apparent in Figure \ref{fig:rh_psi_P25E2E1} and 
...
strengthens as the resolution increases and transports more moisture to the mid-troposphere.
It is also consistent with weaker mixing from shallow clouds as resolution decreases as
discussed in \citeA{Pauluis2006}.   
Figure \ref{fig:rh_psi_P25E2E1_lwoff} also shows greater asymmetries 
...
organized and stronger circulation below 500 hPa and the E2 and E1 simulations 
appear much more similar (Figure \ref{fig:rh_psi_P25E2E1}) than they do with LWCRE off. 
...
and vertical mass transport.     Figure \ref{fig:TotCond} shows the domain mean condensate for E25, E2, and E1
...
subcloud layer and a dry bias in the troposphere above.  This is what our results show as well.   

Examining the LWCRE reveals an interesting difference between the coarse experiments with parameterized convection
and the CRM-like experiments.  Interactive LWCRE lead to less upper level ice-condensate for our CRM experiments
with the effect increasing as the resolution increases (Figure \ref{fig:TotCond}).  The opposite occurs for our 
...
The LWCRE off experiments influence the low-levels of the GCM-like experiments, and the upper-level
regions of the CRM-like experiments.     
%the LWCRE results in less condensate 
...
half of the domain.  It appears that these differences in magnitude and regularity are due to the differing low-level 
wind speeds among 
...
for all experiments when the LWCRE is
off.  Atmospheric radiative cooling can be thought of as a proxy for the mean precipitation 
because the cooling is balanced primarily by condensational heating.  Larger values of 
$\overline{\rm{OLR}}$ would correspond to larger values of $\overline{\rm{P}}$.  
This is clearly not the case 
for the E25, P25, and P100 cases.   \textit{The domain mean precipitation rates decrease despite
an increased amount of atmospheric cooling.}  The implication is that the requisite atmospheric 
heating must come from a process other than condensation.  

The solution lies in the energy budget of the surface.  
A fixed SST means that the upward flux of longwave energy
...
Low-level clouds normally serve as a significant source of longwave radiative cooling 
for the atmosphere.  Making these clouds invisible to radiation creates a source of 
effective atmospheric warming.  
...
condensational heating.  The mean precipitation rate actually decreases.  
...
(Figure \ref{fig:TotCond}) 
therefore imply a larger atmospheric heating due to the CRE relative to the E25, P25, and P100 
experiments in which have fewer clouds aloft (Figures \ref{fig:TotCond} and \ref{fig:TotCond_P25P100}).  
When the warming effect of the upper level clouds 
...
interact with longwave radiation in the atmosphere.  Because there are so many more low-level clouds in 
the GCM-like experiments we see a strong response to upwelling radiation from the surface.  In contrast, 
the abundance of upper-level ice condensate, and lack of low-level condensate in the CRM-like 
experiments results in the primary interaction
between clouds and radiation being in the atmosphere below the upper level clouds.       

%The decrease of precipitation for LWCRE off experiments for the P25 and P100 experiments
...
%of domain mean precipitation for E25, P25, and P100 compared to E2 and E1.  
...
Popp and Silvers, 2017 showed that there is dramatically less condensate in the atmosphere and much less precipitation 
(at the equator) 
for LW CRE off experiments (see their figure 1a,b).  Our results here for the E25, P25, and P100 are consistent with 
those of Popp and Silvers, 2017 because the low-level condensate are strongly decreased when the LWCRE is off.  
...
We were struck by three particular changes due strictly do a 4-fold increase in domain width.  A dramatic widening of the precipitation distribution when the LW CRE is off, a shifting of low level clouds upward by more than 100 hPa, and 
a strong dependence of the quantity of precipitation produced by the large-scale scheme on the LW CRE.  
