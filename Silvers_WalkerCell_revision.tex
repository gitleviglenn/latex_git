...
\usepackage{lineno}
%\usepackage[inline]{trackchanges}        % for track changes. finalnew option will compile document with changes incorporated.
\usepackage[finalnew]{trackchanges}   % use this to create the final pdf without the changes shown.
\usepackage{soul}
\linenumbers
...
\begin{keypoints}
  ...
  \item{High and low clouds interact differently with longwave radiation to increase or decrease precipitation, depending on \add[LGS]{the} dominant cloud type.}
...
To do this we simulate a mock-Walker Circulation with a full-physics General Circulation Model (GCM) using idealized boundary conditions.  Our experiments use a doubly-periodic domain with grid-spacing of 1, 2, 25, and 100km.  We thus span the range from General Circulation Models (GCMs) to Cloud-system Resolving Models (CRMs).  Our model is derived from the Geophysical Fluid Dynamics Laboratory (GFDL) atmospheric GCM (AM4.0).  \change[LGS]{Our GCM-like experiments have a large low-level cloud fraction while the CRM-like experiments have more upper-level clouds.}{We find substantial differences in the mock-Walker simulated by our GCM-like and CRM-like experiments.  The CRM-like experiments have more upper level clouds, stronger overturning circulations, and more precipitation.  The GCM-like experiments have a low-level cloud fraction that is up to 20\% larger.}
These differences lead to opposite atmospheric responses to changes in the longwave cloud radiative effect (LWCRE).  Active LWCRE lead to increased precipitation for our GCMs, but decreased precipitation for our CRMs.  The LWCRE leads to a narrower rising branch of the circulation and substantially increases the fraction of precipitation from the large-scale cloud parameterization.  \remove[LGS]{Decreasing the grid-spacing to 1km and 2km results in stronger overturning circulations, more condensate aloft, and less precipitation.}  This work demonstrates that a mock-Walker circulation is a useful generalization of RCE that includes a large-scale circulation.

%We find substantial differences in the mock-Walker simulated by our GCM-like and CRM-like experiments.  The CRM-like experiments have more upper level clouds, stronger overturning circulations, and more precipitation.  The GCM-like experiments have a low-level cloud fraction that is up to 20\% larger.  
...
ranging from meters to thousands of \change[LGS]{km's}{kilometers} all of which interact with each of the different cloud types.
...
This circulation, now known as the Walker Circulation, is a response to \remove[LGS]{the} 
longitudinal asymmetries in \remove[LGS]{the} tropical atmospheric heating and is tightly coupled with 
the El Ni\~{n}o-Southern Oscillation (ENSO).    
The Walker Circulation \change[LGS]{is a compelling example} 
{is composed}
of both thermodynamic and dynamic interactions
between moisture and the large-scale circulation.  
\change[LGS]{It is also a framework that can be compared 
...
Recent work has shown that the interactions between clouds, patterns of \change[LGS]{S}{s}ea \change[LGS]{S}{s}urface \change[LGS]{T}{t}emperature (SST), and the circulation 
...
several questions that are critical to a better understanding of \remove[LGS]{the} tropical climate and cloud processes.    
These questions include: 
\begin{itemize}
  \item{How do clouds influence the overturning circulation?}
  \item{To what extent are \remove[LGS]{the} deep convective clouds and \remove[LGS]{the} low-level clouds coupled through 
  \remove[LGS]{the} overturning circulations?}
...
both a GCM-like model and a CRM-like model.
 Idealized models of the Walker circulation were \change[LGS]{first called}{referred to as}  `mock-Walker circulations' by \citeA{Raymond_1994}.  
...
% As a result, the distinction between these two types of models
% Accordingly, the parameter space of these two types of models has begun to overlap.   As a result, 
% there is a need to systematically compare the 
% Nevertheless (or, notwithstanding this,), GCMs and CRMs differ in many ways and there is a need to systematically 
% compare the simulated clouds from both.  
...
%
% This paper uses a mock-Walker Circulation to study the coupling between atmospheric moisture and the large-scale circulation.
% Using a single model base in the context of both a GCM and a CRM illustrates how inextricable the interactions between clouds and radiation are 
%
% Simulating a mock-Walker Circulation in the context of both a GCM and a CRM 
% shows how the interactions of clouds and radiation interact
%
% the mock-Walker Circulation provides a simple dynamical context within which
% the inextricable interactions between clouds and radiation can be shown to influence the
% coupling of moisture with the large-scale circulation.
%
% illustrates how inextricable the interactions between clouds and radiation is to the coupling 
% of moisture with the large-scale circulation. 
...
models of \remove[LGS]{the} tropical circulations, and to many \remove[LGS]{of the} recent studies of radiative convective equilibrium (RCE).   
...
towards a CRM.  
\change[LGS]{Our initial motivation for using the GFDL AM4.0 model on a doubly-periodic domain was to simulate a tropical Pacific-like region 
in the context of both a GCM and a CRM in the hopes that resolving more of the turbulent motions and circulations
would help us to better understand }{By simulating a tropical Pacific-like region in GCM-like and CRM-like models, we are able to better understand }    
...
section \change[LGS]{6}{six} includes a brief discussion and lists some of the impacts of the LWCRE and changing 
...
humidity (q), liquid (ql), \remove[LGS]{and} frozen water (qi), and cloud fraction.  The top of the model domain is at 1 hPa, with 33 vertical 
...
One technique that has been commonly used to infer the influence of clouds on the atmospheric \add[LGS]{mean state} is to make the clouds invisible to the radiation.  
...
\add[LGS]{Another technique, referred to as `cloud-locking', can be used to infer how interactions between clouds, radiation, and circulation influence the climate variability}  \cite<e.g.,>[]{Radel_etal_2016}. 
\add[LGS]{This method allows one to decouple the cloud radiative effect from the atmospheric state by holding clouds from a control experiment fixed while the rest of the experiment responds to some perturbation.  This paper does not use the 'cloud-locking' method.}
In the AM4 code, we make the clouds invisible to the radiation separately for the longwave (LW) and shortwave radiation.  
...
Throughout this paper the P100 and P25 experiments, with and without the LWCRE, are referred to as `GCM-like' \change[LGS]
{and `CRM-like'}{while the E2 and E1 experiments are referred to as `CRM-like'.}  The GCM-like experiments \remove[LGS]{only} differ from traditional GCMs in the non-global domain and lack of rotation.  
...
or fraction of domain that is subsiding at the \change[LGS]{532}{500} hPa level.
...
\subsection{\add[LGS]{General Characteristics of the mock-WalkerCirculation}}
...
the warmer SSTs, while the regions with subsiding circulations are dry \change[LGS]{($< 20\% RH$)}{$RH < 20\% $} above about $900\, \rm{hPa}$.  
...
\begin{figure}
  \centering
      \includegraphics[width=0.96\columnwidth]{walkerfigs/rh_psi_cond_25km_6pan_new.eps}
...
\note[LGS]{The following lines, which appear to have been deleted have been moved to subsection 3.2 and 
slightly rewritten for clarity.}
\remove[LGS]{The mass streamfunction and vertical velocity both show the 
P25 (Figure 1 a,d; Figure 2) }
%\ref{fig:rh_psi_P25vsE25}, 
%\change[LGS]{left panels}{a,d}%; Figure \ref{fig:precip_vertvel})
\remove[LGS]{ experiment to have 
a stronger, more concentrated overturning circulation than either the P25 LWCRE-off (Figure} 
%\ref{fig:rh_psi_P25vsE25} 
\remove[LGS]{b,e) or E25 (Figure} 
%\ref{fig:rh_psi_P25vsE25} 
\remove[LGS]{c,f) experiments.   
The control GCM-like experiment (P25) also has higher RH and more condensate in the convective
...
\change[LGS]{This}{The} default configuration in which the clouds interact with the LW radiation results in a stronger,
...
Because of the \change[LGS]{tight}{energetic} constraints that connect $\overline{\rm{P}}$, atmospheric condensational heating, and 
...
is demonstrated in Figure \ref{fig:precip_dom_mn} with the colored circles \add[LGS]{and diamonds} which show the mean values of $\overline{\rm{P}}$ over the last 4 years (5 months) of the experiments for the GCM-like (CRM-like) experiments.   
...
Despite the same boundary conditions and base model, these experiments have a large range of domain mean precipitation 
...
LWCRE leads to larger $\rm{\overline{P}}$ in the GCM-like experiments but smaller $\rm{\overline{P}}$ in the CRM\add[LGS]{-like} experiments.  
...
\note[LGS]{This paragraph was originally on lines 326-343 but has been moved to accommodate the reorganization of section 3.}
...
or equal to the case with LWCRE-off \remove[LGS]{(with the exception of E25)}.   
...
\subsection{\add[LGS]{Comparison of Experiments with Parameterized or Explicit Convection}}
...
\begin{figure}
  \centering
      \includegraphics[width=0.95\columnwidth]{walkerfigs/MeanStructure_P100P25E25.eps}
...
%The patterns of precipitation in Figure \ref{fig:hov_25km_4pan} show an asymmetric and sporadic evolution in time as well as an oscillating pattern that is remarkably steady 
%in time.  
...
\begin{figure}
  \centering
      \includegraphics[width=0.95\columnwidth]{walkerfigs/hov_precip_25km4pan_huge.eps}
...
% discussing the asymmetry of P25 could/should lead into a discussion of Figure \ref{fig:hov_25km_4pan}...

One of the most prominent features of our GCM-like simulations is \change[LGS]{an asymmetry (relative to the 
symmetric SST distribution) in both the time-dependent and steady-state solutions}{the erratic (relative to the symmetric SST distribution) 
structure of the precipitation field}.  
This \remove[LGS]{asymmetry} is particularly apparent in the P25 experiment on the small domain but is also present on larger domains and 
...
(compare Figure \ref{fig:domdep}a,b to Figure \ref{fig:domdep_lwoff}a,b).  
...
\begin{figure}
  \centering
       \includegraphics[width=0.90\columnwidth]{DomainMeanCond_SinglePanel.pdf}
...
\begin{figure}
  \centering
      \includegraphics[width=0.96\columnwidth]{walkerfigs/rh_psi_cond_E25E2E1_lwon_new.eps}
...
\begin{figure}
  \centering
      \includegraphics[width=0.96\columnwidth]{walkerfigs/rh_psi_cond_E25E2E1_lwoff_new.eps}
...
When the clouds and radiation directly interact with each other the experiments have a \change[LGS]{better}{more}
...
\change[LGS]{It}{The condensate} provides clues about the strength of convection and the vertical mass transport.     
...
\begin{figure}
  \centering
       ...
       \includegraphics[width=0.45\columnwidth]{walkerfigs/DomainMnCond_4mods_new.eps}
...
Interactive LWCRE leads to less upper level ice-condensate for our CRM\add[LGS]{-like} experiments
...
in the GCM\add[LGS]{-like} experiments, but a negligible decrease in the condensate of the CRM\add[LGS]{-like experiments}.    
...
by these characteristics differ dramatically between the GCM\add[LGS]{-like} and CRM\add[LGS]{-like} experiments.   

\begin{figure}
  \centering       
    \includegraphics[width=0.96\columnwidth]{walkerfigs/lowlevel_New_b.eps}
...
\begin{figure}
  \centering
      ...
      \includegraphics[width=0.96\columnwidth]{walkerfigs/CF_tdtlw_4pan_fig.eps}
...
for all LWCRE-off experiments.  \add[LGS]{Net} atmospheric radiative cooling can be thought of 
as a proxy for the mean precipitation because the cooling is usually balanced primarily by 
condensational heating.  \add[LGS]{All else remaining equal, }larger values of $\overline{\rm{OLR}}$ would then correspond to 
...
amount of \change[LGS]{atmospheric cooling}{$\overline{\rm{OLR}}$}.  The implication is that the requisite atmospheric heating 
must come from a process other than condensation.  

\begin{figure}
  \centering
      %\includegraphics[width=0.96\columnwidth]{walkerfigs/CloudFractdtRad_lwcre.eps}
      \includegraphics[width=0.96\columnwidth]{walkerfigs/Silvers_WalkerCell_MISR.pdf}
...
Low-level clouds \add[LGS]{, especially in dry regions,} serve as a significant source of \remove[LGS]{LW} radiative cooling for the atmosphere (Figure \ref{fig:cf_tdtlw}) \add[LGS]{because they increase the downwelling longwave radiation}.  
...
The large decrease of low-level clouds \add[LGS]{when the LWCRE is off} also leads to an increase of downward shortwave radiation at the surface.  
...
\add[LGS]{In this section we conclude by summarizing the primary results of our study, followed by a brief discussion 
...
Over the regions with cooler SSTs, the CRMs have very few (less than \change[LGS]{\%5}{5\%}) low-level clouds and as a result the 
change of $\overline{\rm{P}}$ is driven by the increased LW cooling to space in the LWCRE-off case.
\citeA{Watanabe_etal_2018} found a similar relationship between low-clouds and precipitation in the context \add[LGS]{of} climate 
...
\end{itemize}
%The resulting atmospheric state has a stronger overturning 
...
\begin{itemize}
 ...
 {\remove[LGS]{When convection is parameterized the total domain mean condensate increases at all heights 
  (Figure 6), in contrast to when the convection is explicit and 
  the condensate aloft decreases (Figure 11.}    }
\end{itemize}
...
Two examples include the \change[LGS]{consistent asymmetry of the}{consistently off-center} circulation\add{s}, and precipitation \add{patterns}, 
